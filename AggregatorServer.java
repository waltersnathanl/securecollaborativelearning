...
public class AggregatorServer{
    public static void main(String[] args){
        ...
        InetAddress[] clients = new InetAddress [3]; //TODO: put in the IP addresses
        ...
        for(int i=0,i<keys.length,i++){
            privateKeys[i] = keys[i].getPrivateKey();
        }
        ...
        for(int i=0,i<clients.length;i++){
            try (Socket socket = new Socket(clients[i],8080)){
                ...
                objectOutputStream.writeObject(privateKeys[i]);
        ...
        for(int i=0,i<clients.length;i++){
            try (Socket socket = new Socket(clients[i],8080)){
                ...
                currentResponse = (BigInteger[length_of_response]) objectInputStream.readObject();
                responseMatrix[i] = currentResponse;
            }
        }
        ...
        for(int j=0;j<length_of_response;j++){
            DecryptionZKP[] partialDecryptions = new DecryptionZKP[number_of_clients];
            for(int i=0,i<clients.length;i++){
                try (Socket socket = new Socket(clients[i], 8080)) {
                    OutputStream outputStream = socket.getOutputStream();
                    InputStream inputStream = socket.getInputStream();
                    ...
                    currentResponse = (DecryptionZKP) objectInputStream.readObject();
                    partialDecryptions[i] = currentResponse;
                }
            }
        ...
        double oddsRatio = double(cleartextAggregates[0]*cleartextAggregates[3]/(double(cleartextAggregates[1]) * double(cleartextAggregates[2]));
